...
include_once dirname(__DIR__) . '/modelo/Ciudad.inc.php';
include_once dirname(__DIR__) . '/modelo/CiudadDaoFactory.inc.php';
include_once dirname(__DIR__) . '/modelo/CiudadValidador.inc.php';
...
define('MODULO', 'ciudad');
define('RUTA_ADMINISTRAR', RUTA_ADMINISTRAR_CIUDAD);
define('RUTA_MANTENER', RUTA_MANTENER_CIUDAD);
    ...
    if ($peticion === 'crear' || $peticion === 'actualizar') {
        ...
        if ($validador->es_valido()) {
            ...
            if (!is_null($modelo) && !is_null($dto)) {
                ...
                if ($peticion === 'crear') {
                    $respuesta = $repositorio->agregar(
                        $_SESSION['cod_usuario'], $dto);
                } elseif ($peticion === 'actualizar') {
                    $respuesta = $repositorio->modificar(
                        $_SESSION['cod_usuario'], $dto);
                }
            }
        }
