...
include_once 'app/modulos/maquina/modelo/Maquina.inc.php';
include_once 'app/modulos/maquina/modelo/MaquinaDaoFactory.inc.php';
include_once 'app/modulos/marca_ot/modelo/MarcaOt.inc.php';
include_once 'app/modulos/marca_ot/modelo/MarcaOtDaoFactory.inc.php';
include_once dirname(__DIR__) . '/modelo/modelo.inc.php';
include_once dirname(__DIR__) . '/modelo/modeloDaoFactory.inc.php';
include_once dirname(__DIR__) . '/modelo/modeloValidador.inc.php';
...
define('MODULO', 'modelo');
define('RUTA_ADMINISTRAR', RUTA_ADMINISTRAR_MODELO);
define('RUTA_MANTENER', RUTA_MANTENER_MODELO);
...
$repositorio_maquina = MaquinaDaoFactory::crear_dao();
$repositorio_marca = MarcaOtDaoFactory::crear_dao();

if (is_null($repositorio) || is_null($repositorio_maquina) ||
        is_null($repositorio_marca))
    Redireccion::redirigir(RUTA_ADMINISTRAR);
    ...
    if ($peticion === 'crear' || $peticion === 'actualizar') {
        ...
        if ($validador->es_valido()) {
            ...
            if (!is_null($modelo) && !is_null($dto)) {
                ...
                $dto->EstablecerMaquina($modelo->obtener_maquina());
                $dto->EstablecerMarca($modelo->obtener_marca());
...
*/
if ($peticion === 'crear' || $peticion === 'actualizar') {
    $maquinas = $repositorio_maquina->obtener_todos('vigente');
    $marcas = $repositorio_marca->obtener_todos('vigente');
} elseif ($peticion === 'leer' || $peticion === 'borrar') {
    if (isset($modelo) && is_object($modelo)) {
        $cod_maquina = $modelo->obtener_maquina();
        $cod_marca = $modelo->obtener_marca();
    }

    $maquina = $repositorio_maquina->obtener_por_codigo($cod_maquina);
    $marca = $repositorio_marca->obtener_por_codigo($cod_marca);
